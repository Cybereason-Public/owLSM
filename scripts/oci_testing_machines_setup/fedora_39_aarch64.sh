...
# Set root password.
# Fedora defaults to yescrypt ($y$), which makes sshd log
# "Could not get shadow information for root" and reject password auth
# even when PermitRootLogin/PasswordAuthentication are yes (su still works).
# Force SHA512 so sshd's shadow path can verify the hash.
echo "root:Password1" | chpasswd -c SHA512
...
# Set admin password (same SHA512 reason as root)
echo "admin:Password1" | chpasswd -c SHA512
...
# 00- wins over vendor drop-ins (e.g. 40-redhat-crypto-policies.conf,
# 50-cloud-init.conf) that may reappear on first boot.
# UsePAM yes is required: without it sshd cannot read /etc/shadow for password
# auth and fails with "Could not get shadow information for root".
