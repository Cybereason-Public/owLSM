...
11. Container image name + tag: For testing use the names specified in `Plan/agent_testing_tools.md` and for local use owlsm:local. Official image is owlsm-runtime (1c).
...
- Linux vs K8s build flavors — endpoint tarballs must never contain Go
    ...
    + `make test`: Linux stub, do not link `libowlsm_k8s.so`. No KubernetesClient unit tests. Pod-cache tests stay. If needed use the `kubernetes_client_stub.cpp` as well.
    + `kubernetes/kind/setup.sh` builds the local runtime image from `build/owlsm-k8s`


- Events from owLSM container should be ignored. 
    + Ignore events that are related to owlsm (either with container based filtering or namespace based filtering)
    + Ignore `kube-system` events
    + by default don't ignore host events (not from a container), but have this option in the config.

### 1c. Official K8s image + chart (same version as Linux)
Linux tarball, `owlsm-runtime` image, and Helm chart are one version (e.g. `v0.9.0`). Image is the K8s-flavored build, not the Linux tarball.

- Helm chart version `X.Y.Z` uses image `owlsm-runtime:vX.Y.Z`. Image repo/tag are not user-overridable.
- `build-artifacts-full-lifecycle.yml`: after the existing Linux build, `make K8S=1` → `build/owlsm-k8s` (amd64 only for now), build `owlsm-runtime` from that dir, `docker save` to a tar, `helm package` the chart. Keep uploading Linux tarballs as today; also upload the image tar and packaged chart as Actions artifacts. Do not publish (no GitHub Release, no GHCR push).
- `publish-release.yml`: same version for all three — Linux tarballs on the GitHub Release (unchanged), `owlsm-runtime:vX.Y.Z` on GHCR, Helm chart on GHCR (chart `vX.Y.Z` → image `owlsm-runtime:vX.Y.Z`).
- Do not run the first real GHCR publish until 1d kind smoke has passed.
...
- K8s aarch64 / multi-arch `owlsm-runtime` (Phase 4)

---

## Suggested order of work
1a → 1b → 1c → 1d → 2a → 2b → 3 → 4 (pick pieces as needed)
